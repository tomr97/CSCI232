...
public class WorstFitDecreasing {
     ...
     **********************************************************************************/
    public WorstFitDecreasing(int[] _in, int _c)
    {
        input_arr = _in;
    ...
    {
        System.out.println("\n");
        System.out.println(" Worst Fit Decreasing Results:");
        System.out.println("-------------------------------");
    ...
    private void run()
    {
        ...
        {
            worst_diff   = 0;
            i_worst_diff = -1; // Sets index for best difference to be out of bounds
            pointer = start;
            
            do
            {                
                int temp = pointer.get_space_left();
                if(( temp - input_arr[i] ) > 0 && ( temp - input_arr[i] ) > worst_diff )
                {
                    worst_diff   = temp;
                    i_worst_diff = pointer.get_bin();
            ...
            else if(i_worst_diff > 0)
            {
                for(int j = 0; j < i_worst_diff; j++)
                {
                    pointer = pointer.get_next();
                }
                pointer.add_job(input_arr[i]);
